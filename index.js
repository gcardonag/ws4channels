...
async function startTranscoding() {
  ...
  ffmpegProc = ffmpeg()
    ...
    .inputOptions(['-f concat','-safe 0','-stream_loop -1','-re'])
    .complexFilter([`[0:v]scale=${VIEW_DIMENSIONS.width}:${VIEW_DIMENSIONS.height}[v]`,'[1:a]volume=0.5[a]'])
    .outputOptions(['-map [v]','-map [a]','-c:v libx264','-pix_fmt yuv420p','-c:a aac','-b:a 128k','-preset ultrafast',`-g ${FRAME_RATE * HLS_SEGMENT_SECONDS}`,'-b:v 1000k','-f hls',`-hls_time ${HLS_SEGMENT_SECONDS}`,'-hls_list_size 2','-hls_flags delete_segments'])
    .output(HLS_FILE)
